...
impl SerialPort {
    ...
    fn is_transmit_empty(&self) -> u8 {
        return port_byte_in(self.port + 5) & 0x20;
    }
    ...
    pub fn init(&self) -> Result<(), ()> {
        ...
        port_byte_out(self.port + 0, 0x03); // Set divisor to 3 (lo byte) 38400 baud
        ...
        port_byte_out(self.port + 0, 0xAE); // Test serial chip (send byte 0xAE and check if serial returns same byte)

        // Check if serial is faulty (i.e: not same byte as sent)
        if port_byte_in(self.port + 0) != 0xAE {
            return Err(());
        }
